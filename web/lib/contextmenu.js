...
function EventRightClick() { /* Closure for this */
	var thisObject = this;
	...
	this.MouseMenu = {
		...
		display: function(e, button) {
			...
			var mouseMenu = document.getElementById('contextMenu');
			if (button === "RIGHT") {
	...
	this.MouseButton = {
		targetContainer: document.getElementById('whiteBoard'),
		menuContainer: document.getElementById('contextMenu'),
		overTarget: false,
		...
		/* End */
		end: function(e) { /* Prevent Event-Bubbling */
...
var domReadyEvent = {
	...
	add: function(handler) { /* Assign each event handler a unique ID. If the handler has an ID, it has already been added to the events object or been run.*/
	...
	remove: function(handler) { /* Delete the event handler from the hash table */
		if (handler.$$domReadyID) {
	...
	run: function() { /* quit if this function has already been called */
		if (this.bDone) {
			return;
		}
	...
	schedule: function() { /* Quit if the init function has already been called*/
		if (this.bDone) {
			return;
		}
		...
		} else if (document.getElementById("__ie_onload")) { /* Second, check for IE.*/
			return true;
		}
		...
		if (typeof this.DOMContentLoadedCustom === "function") {
			...
			if (typeof document.getElementsByTagName !== 'undefined' && (document.getElementsByTagName('body')[0] !== null || document.body !== null)) { /* Call custom function. */
	...
	init: function() { /* If addEventListener supports the DOMContentLoaded event.*/
