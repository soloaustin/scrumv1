...
function httpProxy(request,response,uri){
    ...
    var clientRequest = http.request(options,function(clientResponse){
        ...
        clientResponse.on('data',function(chunk){console.log(chunk);response.write(chunk);});
        clientResponse.on('end',function(){response.end();});
    });
    clientRequest.on('error',function(e){unknownError(response,e)});
    clientRequest.end();
